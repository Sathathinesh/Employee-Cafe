...
import { useNavigate, useRouteData } from '@tanstack/router'; // Updated imports
...
const AddEditCafePage = () => {
  const { id } = useRouteData(); // Use useRouteData to access route params
  const isEdit = Boolean(id);
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [openConfirm, setOpenConfirm] = useState(false);

  const { data: cafeData, isLoading } = useQuery(['cafe', id], () =>
    fetchCafes().then((cafes) => cafes.find((cafe) => cafe.id === Number(id))), {
      enabled: isEdit,
    }
  );
  ...
  const mutation = useMutation(isEdit ? updateCafe : createCafe, {
    onSuccess: () => {
      ...
      navigate('/cafes'); // Use navigate for navigation
    },
  });
